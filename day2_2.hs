...
numberVersion :: [[Int]]
numberVersion = map getNumbers matrix

sortedLists :: [[Int]]
sortedLists = map (sortOn Down) numberVersion

divisorResList :: [Int] -> Int
divisorResList list =
  let divisor = head list in
  sum [div divisor dividend | dividend <- (tail list) , mod divisor dividend == 0]
...
findDivision (x:xs)
  | divRes > 0 = divRes
  | otherwise = findDivision xs
  where divRes = (divisorResList (x:xs))

result :: Int
result = sum [findDivision x | x <- sortedLists]
